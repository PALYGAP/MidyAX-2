...
void BCF2000::colorSlider( int sliderIndex )
{
    if( g_sliderColored )
    {
        if( m_BCF2000MappingTable[ sliderIndex ].MIDIValue > 63)
            m_slidderPalette.setColor(m_TheSliders[sliderIndex]->backgroundRole(),
                                     QColor(
                                         255,
                                         255 - ( (m_BCF2000MappingTable[ sliderIndex ].MIDIValue -63) * 3.9),
                                         255 - ( (m_BCF2000MappingTable[ sliderIndex ].MIDIValue -63) * 3.9),
                                         255)  );
         else
            m_slidderPalette.setColor(m_TheSliders[sliderIndex]->backgroundRole(),
                                  QColor(
                                         255 - ( (63 - m_BCF2000MappingTable[ sliderIndex ].MIDIValue) * 3.9),
                                         255 - ( (63 -m_BCF2000MappingTable[ sliderIndex ].MIDIValue) * 3.9),
                                         255,
                                         255  )       )
        ;
...
void BCF2000::setParameterMapping( void)
{
    ...
    if ( m_dlgParamMapping->exec() == QDialog::Accepted )
    {
        ...
        // If the BCF2000 Knob/Slidder was allready mapped
            ...
            for( int j=0 ; j < m_pInstance->m_numbOfMappings ; j++)
            {
                         ...
                         &&   m_pInstance->m_theMappingTable[j].effectParameterID == prevEffectParameterID)
                {
                    ...
                    m_pInstance->sendSYSEXtoAXEFX(AXEFX_SYSEX_PARAM_SET_Q,
                                                m_pInstance->m_theMappingTable[j].effectBlockID,
                                                m_pInstance->m_theMappingTable[j].effectParameterID);
        ...
        m_pInstance->fillCCtoAXE();
        //m_pInstance->getMidiMappingParamValues();
